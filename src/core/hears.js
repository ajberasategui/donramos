...
module.exports = function(theBot, theController, theConfig, memory, botName, hearDir) {
    myName = botName;
    bot = theBot;
    ...
    if (!_.isUndefined(hearsDir)) {
        ...
        _.forEach(extHears, function(hear) {
            if (-1 !== hear.indexOf('.hear.js')) {
                let requireFile = config.HEARS_DIR_REL_TO_CORE + _.replace(hear, ".js", "");
                logger.log("Trying to load: " + requireFile);
                var h = require(requireFile);
                try { 
...
function meVoy(bot, message) {
    ...
    drMemory.rememberThat("usersInRG", usersInRG);
}
...
function quienEnRG(bot, message) {
    drMemory.recall("usersInRG")
        .then((usersInRG) => {
            if (_.isEmpty(usersInRG)) {
                bot.reply(message, "Que yo sepa, _naides_");
                return;
            }
            var users = '';
            for (var i = 0; i < usersInRG.length; i++) {
                controller.storage.users.get(usersInRG[i], (err, user) => {
                    if (err) {
...
function saveNewConcept(question, answer) {
    // This function does not belongs here.
    drMemory.recall("learnt", (err, concepts) => {
        if (err) {
        ...
        db.rememberThat("learnt", concepts);
    });
}
